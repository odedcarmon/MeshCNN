...
def define_classifier(input_nc, ncf, ninput_edges, nclasses, opt, gpu_ids, arch, init_type, init_gain):
    ...
    if arch == 'mconvnet':
        net = Team1_MeshConvNet(norm_layer, input_nc, ncf, nclasses, ninput_edges, opt.pool_res, opt.fc_n,
                          opt.resblocks)
    elif arch == 'meshunet':
        down_convs = [input_nc] + ncf
        up_convs = ncf[::-1] + [nclasses]
        pool_res = [ninput_edges] + opt.pool_res
        net = Team1MeshEncoderDecoder(pool_res, down_convs, up_convs, blocks=opt.resblocks,
                                 transfer_data=True)
    else:
...
class Team1MeshEncoder(nn.Module):
    def __init__(self, pools, convs, fcs=None, blocks=0, global_pool='max'):
        super(Team1MeshEncoder, self).__init__()
        self.fcs = None
        self.convs = []
...
class Team1DownConv(nn.Module):
    def __init__(self, in_channels, out_channels, blocks=0, pool=0):
        super(Team1DownConv, self).__init__()
        self.bn = []
        self.pool = None
...
class Team1MeshEncoderDecoder(nn.Module):
    """
    Team1s' new Network for fully-convolutional tasks (segmentation)
    """
    def __init__(self, pools, down_convs, up_convs, blocks=0, transfer_data=True):
        super(Team1MeshEncoderDecoder, self).__init__()
        self.transfer_data = transfer_data
        self.encoder = Team1MeshEncoder(pools, down_convs, blocks=blocks)
        unrolls = pools[:-1].copy()
        unrolls.reverse()
        self.decoder = Team1MeshDecoder(unrolls, up_convs, blocks=blocks, transfer_data=transfer_data)
